...
<div class="container">
    ...
    <div class="card" style="border:2px solid <?php echo !empty($payment['AccountNumber']) ? '#28a745' : '#ffc107'; ?>;">
        <div class="card-header" style="background:<?php echo !empty($payment['AccountNumber']) ? '#f0fff4' : '#fffbf0'; ?>;">
            <div class="card-icon" style="background:<?php echo !empty($payment['AccountNumber']) ? '#d4edda' : '#fff3cd'; ?>;">&#127981;</div>
            <h2>Employee Bank Account (Pay to this account)</h2>
            <?php if (!empty($payment['AccountNumber'])): ?>
            <span style="background:#d4edda;color:#155724;padding:3px 10px;border-radius:12px;font-size:11px;font-weight:700;margin-left:auto;">&#9989; Verified on file</span>
            <?php else: ?>
            <span style="background:#fff3cd;color:#856404;padding:3px 10px;border-radius:12px;font-size:11px;font-weight:700;margin-left:auto;">&#9888; Not provided</span>
            <?php endif; ?>
        </div>
        <div class="card-body">
            <?php if (!empty($payment['AccountNumber'])): ?>
            <div class="info-grid">
                <div class="info-item"><label>Bank Name</label>
                    <div class="val" style="font-size:16px;font-weight:700;"><?php echo htmlspecialchars($payment['BankName'] ?? '—'); ?></div>
                </div>
                <div class="info-item"><label>Account Number</label>
                    <div class="val" style="font-size:22px;font-weight:800;font-family:monospace;letter-spacing:3px;color:#28a745;">
                        <?php echo htmlspecialchars($payment['AccountNumber']); ?>
                    </div>
                </div>
                <div class="info-item"><label>Account Name</label>
                    <div class="val" style="font-size:16px;font-weight:700;"><?php echo htmlspecialchars($payment['AccountName'] ?? '—'); ?></div>
                </div>
            </div>
            <div style="margin-top:14px;background:#e8f5e9;padding:12px 16px;border-radius:8px;font-size:13px;color:#155724;">
                &#128276; <strong>Important:</strong> Please verify this account number before processing the payment of <strong>&#8358;<?php echo number_format($payment['Amount'],2); ?></strong>.
            </div>
            <?php else: ?>
            <div style="text-align:center;padding:20px;background:#fffbf0;border-radius:9px;color:#856404;">
                <p><strong>&#9888; Employee has not provided bank details.</strong></p>
                <p style="font-size:13px;margin-top:6px;">Ask the employee to update their bank details in their portal before processing payment.</p>
            </div>
            <?php endif; ?>
        </div>
    </div>
