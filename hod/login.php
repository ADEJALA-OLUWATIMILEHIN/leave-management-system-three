...
if (is_logged_in()) {
    if (is_hod()) {
        redirect('index.php'); // Already HOD, go to dashboard
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($password)) {
        ...
        // Query to check HOD credentials - ONLY hod role (ADDED MustChangePassword)
                ...
                if (password_verify($password, $user['Password'])) {
                    ...
                    if ($user['MustChangePassword'] == 1) {
                        ...
                        $log_sql = "INSERT INTO ActivityLog (UserID, Action, Description, IPAddress) 
                                    VALUES (?, 'HOD Login', 'HOD logged in - Password change required', ?)";
                    ...
                    // Log the login
                    $log_sql = "INSERT INTO ActivityLog (UserID, Action, Description, IPAddress) 
                                VALUES (?, 'HOD Login', 'HOD logged in', ?)";
                    $log_params = array($user['UserID'], $_SERVER['REMOTE_ADDR']);
                    sqlsrv_query($conn, $log_log_params);
...
<head>
    ...
    <title>HOD Login - Leave Management</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="login-container">
        <div class="login-header">
            <div class="hod-badge">HOD Portal</div>
            <h1>Leave Management</h1>
            <p>Head of Department Access</p>
        </div>
    ...
    <a href="../index.php">← Back to Main Portal</a>
        ...
        <form method="POST" action="">
            <div class="form-group">
                <label for="email">HOD Email</label>
                <input 
                    type="email" 
                    id="email" 
                    name="email" 
            ...
            <button type="submit" class="btn-login">HOD Sign In</button>
        </form>
        
        <div class="other-links">
            <p>Other Portals:</p>
            <a href="../login.php">Employee</a> | 
            <a href="../hr/login.php">HR</a>
        </div>
    </div>
</body>
</html>